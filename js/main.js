...
const VERSION_OPTIONS = {
  ...
  bedrock: [
    ["bedrock-latest", "統合版 最新"],
    ["bedrock-1.21", "統合版 1.21系"],
    ["bedrock-1.20", "統合版 1.20系"],
  ],
};
...
elements.edition.addEventListener("change", () => {
  renderVersionOptions(elements.edition.value);
  updateVersionNote();
});
...
renderVersionOptions(elements.edition.value, elements.version.value || "java-1.21");
...
function generateMap(successMessage = "マップを生成しました。チャンクを選択すると詳細を確認できます。") {
  ...
  const autoStructureNote = elements.autoStructureLayerToggle.checked
    ? ` / 構造物候補 ${autoStructureCount}件（候補表示）`
    : "";
  const slimeNote = elements.slimeLayerToggle.checked ? ` / スライム ${slimeCount}件` : " / スライム非表示";
...
function getVisibleStructureRecords() {
  return getVisibleStructures({
    ...
    showManual: elements.manualMarkerLayerToggle.checked,
    showAuto: elements.autoStructureLayerToggle.checked,
  });
}

function applySlimeLayerDisplay() {
  elements.grid.classList.toggle("hide-slime", !elements.slimeLayerToggle.checked);
}
...
function updateSlimeLayerToggleLabel() {
  ...
  label.textContent = elements.slimeLayerToggle.checked ? "スライムチャンク: ON" : "スライムチャンク: OFF";
}
...
function updateAutoStructureLayerToggleLabel() {
  ...
  label.textContent = elements.autoStructureLayerToggle.checked ? "構造物候補: ON" : "構造物候補: OFF";
}
...
function updateManualMarkerLayerToggleLabel() {
  ...
  label.textContent = elements.manualMarkerLayerToggle.checked ? "手動マーカー: ON" : "手動マーカー: OFF";
}
...
function updateTerrainLayerToggleLabel() {
  ...
  label.textContent = elements.terrainLayerToggle.checked ? "簡易地形: ON" : "簡易地形: OFF";
}
...
function updateVersionNote() {
  if (elements.edition.value === "bedrock") {
    elements.versionNote.textContent = "統合版 最新 / 1.21系 / 1.20系 は現時点で同一候補ロジックです。候補表示として扱います。";
    return;
  }
  elements.versionNote.textContent = "Java版 1.21 / 1.20 / 1.19 / 1.18 は現時点で同一候補ロジックです。";
}
